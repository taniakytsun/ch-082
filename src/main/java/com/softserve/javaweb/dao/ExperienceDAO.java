...
public class ExperienceDAO implements DAO<Experience> {
    ...
    private static String readOlByPersonId = "SELECT * FROM experience WHERE idperson = ?";
    ...
    public List<Experience> readAllByPersonId(Long personId){
        List<Experience> experiences = new ArrayList<>();
        try(PreparedStatement preparedStatement = connection.getConnection().prepareStatement(readOlByPersonId)){
            preparedStatement.setLong(1, personId);
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                Experience experience = new Experience();
                while (resultSet.next()) {
